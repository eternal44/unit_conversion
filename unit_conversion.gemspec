# coding: utf-8
lib = File.expand_path('../lib', __FILE__)
$LOAD_PATH.unshift(lib) unless $LOAD_PATH.include?(lib)
require 'unit_conversion/version'

Gem::Specification.new do |spec|
  spec.name          = "unit_conversion"
  spec.version       = UnitConversion::VERSION
  spec.authors       = ["James Youn"]
  spec.email         = ["jamesyoun710@gmail.com"]

  spec.summary       = %q{Unit conversions}
  spec.homepage      = "https://github.com/eternal44/unit_conversion"
  spec.license       = "MIT"

  spec.files         = `git ls-files -z`.split("\x0").reject { |f| f.match(%r{^(test|spec|features)/}) }
  spec.bindir        = "exe"
  spec.executables   = spec.files.grep(%r{^exe/}) { |f| File.basename(f) }
  spec.require_paths = ["lib"]

  spec.add_development_dependency "bundler", "~> 1.10"
  spec.add_development_dependency "rake", "~> 10.0"
  spec.add_development_dependency 'minitest', '~> 5.8', '>= 5.8.1'
  spec.add_development_dependency 'minitest-reporters', '~> 1.1', '>= 1.1.3'
  spec.add_development_dependency "guard", "~> 2.13"
  spec.add_development_dependency 'guard-minitest', '~> 2.4', '>= 2.4.4'
  spec.add_development_dependency 'terminal-notifier-guard', '~> 1.6', '>= 1.6.4'
end
